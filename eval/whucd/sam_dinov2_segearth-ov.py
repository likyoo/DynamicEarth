...
TEXT_PROMPT = ['background', 'building']
INPUT_DIR = 'data/WHU-CD-256/test'
OUTPUT_DIR = 'output/WHU-CD-256_exp/test_sam_dinov2_segearth-ov'
...
mp.make_mask_generator(
    model=sam,
    points_per_side=8, # for 256*256 image
    points_per_batch=64,
    pred_iou_thresh=0.5,
...
identifier_model, identifier_processor = get_identifier('SegEarth-OV', DEVICE, name_list=TEXT_PROMPT,
                                                        processor_config={'resize': (224, 224)})

# Process each image pair in the input directory
for file_name in tqdm(os.listdir(os.path.join(INPUT_DIR, 'A')), desc="Processing", unit="iteration"):
    ...
    cmasks, img1_mask_num = bitemporal_match(img1, img2, masks, comparator_model, comparator_processor,
                                             img1_mask_num, change_confidence_threshold=125, device=DEVICE,
