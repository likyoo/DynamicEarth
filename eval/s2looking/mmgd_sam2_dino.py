...
TEXT_PROMPT = 'roof. rooftop. building. house. apartment. residential. factory' # separated by .
INPUT_DIR = 'data/S2Looking/test_256'
OUTPUT_DIR = 'output/S2Looking_exp/test_mmgd_sam2_dino'
...
for file_name in tqdm(os.listdir(os.path.join(INPUT_DIR, 'Image1')), desc="Processing", unit="iteration"):
    # Construct file paths for the images
    img1_path = os.path.join(INPUT_DIR, 'Image1', file_name)
    img2_path = os.path.join(INPUT_DIR, 'Image2', file_name)
    ...
    if len(img1_boxes) == 0 and len(img2_boxes) == 0:
        ...
        cmasks, img1_mask_num = bitemporal_match(img1, img2, cmasks, comparator_model, comparator_processor,
                                             img1_mask_num, change_confidence_threshold=130, device=DEVICE,
